...
const GuidedSearchForm = ({ onSearch, isSearching, compact = false }) => {
  ...
  // Clean, single-trigger auto-focus system (no over-triggering)
  useEffect(() => {
    ...
    // Single trigger for date picker after both origin and destination are set
    if (origin && dest && !dep) {
                ...
                {trendingRoutes.map((route, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-bold text-blue-600 trending-price group-hover:text-blue-700">{route.price}</div>
                        <div className="text-xs text-gray-500">onwards</div>
                      </div>
                    </div>
                  </button>
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      <main className="w-full px-2 sm:px-4 md:px-6 lg:px-8 py-4 md:py-8 max-w-7xl md:mx-auto">
