...
function SearchCard({ onSearch, overlayStates, searchStates, guidedFlow }) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4">
      <div className="rounded-2xl shadow-xl border border-blue-100/30 bg-gradient-to-br from-white to-blue-50/20 p-4 md:p-6 relative overflow-hidden group hover:shadow-2xl transition-all duration-500">
        ...
        <div className="mt-3 compact-flight-container" style={{ 
          ...
          <div 
            ...
            style={{ 
              ...
              cursor: 'pointer',
              borderRadius: '10px',
              border: getStepStatus(2) === 'active' ? '2px solid #3B82F6' : '1px solid #d1d5db'
            }}
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
...
function App() {
  // Navigation state
  const [currentPage, setCurrentPage] = useState('results'); // 'search', 'results', 'selection', 'passenger', 'payment', 'confirmation' - TEMP: Show results for visual enhancement
