...
const FlightResults = ({ searchParams, onFlightSelect }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-gray-100">
      ...
      <header className="bg-white/95 backdrop-blur-sm border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            {/* WhatsApp Support & Sign In - Smaller, Secondary to Logo */}
      ...
      {/* Sticky Top Bar with Search Summary and Modify Button */}
      <div className="bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 sticky top-[60px] z-30 shadow-md">
        <div className="max-w-7xl mx-auto px-4 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-3 sm:gap-4">
            {/* Search Summary */}
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap text-sm sm:text-base">
              <div className="flex items-center gap-1 sm:gap-2">
                <span className="text-xs sm:text-sm font-semibold text-gray-700">From:</span>
                <span className="text-sm sm:text-base font-bold text-gray-900">{searchParams?.from?.city || 'Mumbai'}</span>
              </div>
              <span className="text-gray-400 hidden sm:inline">→</span>
              <span className="text-gray-400 sm:hidden">to</span>
              <div className="flex items-center gap-1 sm:gap-2">
                <span className="text-xs sm:text-sm font-semibold text-gray-700 hidden sm:inline">To:</span>
                <span className="text-sm sm:text-base font-bold text-gray-900">{searchParams?.to?.city || 'Delhi'}</span>
              </div>
              <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-white rounded-full">
                <svg className="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span className="text-sm text-gray-700">{searchParams?.departDate || 'Today'}</span>
              </div>
            </div>
            
            {/* Modify Search Button - Subtle & Clean */}
            <button
              onClick={() => setShowSearchOverlay(true)}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-200 font-medium shadow-sm hover:shadow-md whitespace-nowrap text-xs sm:text-sm"
            >
      ...
      {/* Original Date Display (Hidden) */}
      <div className="bg-white border-b border-gray-200 sticky top-[60px] z-30" style={{display: 'none'}}>
        <div className="max-w-7xl mx-auto px-4 py-3">
          {/* Route and Date Info */}
                ...
                <div className="text-center py-12 px-4">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md mx-auto text-left">
                    ...
                    <ul className="text-xs sm:text-sm text-blue-800 space-y-1">
                      <li>• Delhi ↔ Bangalore</li>
                      <li>• Mumbai → Chennai</li>
                      <li>• Bangalore → Delhi</li>
                    </ul>
                    <p className="text-xs text-blue-700 mt-3">Try searching these routes to see live flight data.</p>
                  </div>
                </div>
              ) : (
